...
class Visualizer:
    ...
    def __init__(self, mode: VisualizationMode, task: TaskType) -> None:
        if mode not in (VisualizationMode.FULL, VisualizationMode.SIMPLE, VisualizationMode.HEATMAP):
            raise ValueError(f"Unknown visualization mode: {mode}. Please choose one of ['full', 'simple', 'heatmap']")
        self.mode = mode
    ...
    def visualize_batch(self, batch: dict) -> Iterator[np.ndarray]:
        ...
        if "pred_scores" in batch:
            ...
            scores = batch["pred_scores"].detach().clone().tolist()
    ...
    def visualize_image(self, image_result: ImageResult) -> np.ndarray:
        ...
        if self.mode == VisualizationMode.HEATMAP:
            return self._visualize_heatmap(image_result)
    ...
    def _visualize_heatmap(self, image_result: ImageResult) -> np.ndarray:
        """Generate a single-channel heatmap.

        Args:
            image_result (ImageResult): GT and Prediction data for a single image.

        Returns:
            np.ndarray: The single-channel heatmap.
        """
        if self.task == TaskType.SEGMENTATION:
            img = image_result.anomaly_map * 255
            if len(img.shape) == 3:
                img = img.transpose((1, 2, 0))
            return (img, self._visualize_simple(image_result))
        else:
            raise ValueError(f"Cannot generate heatmap with task type {self.task}")
