...
class AnomalyModule(pl.LightningModule, ABC):
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        # increase range of anomaly maps for display
        if "Padim" in self.__class__.__name__:
            outputs["anomaly_maps"] *= 4
            outputs["anomaly_maps"] = min(255, outputs["anomaly_maps"].max()) * (outputs["anomaly_maps"] - outputs["anomaly_maps"].min()) / (outputs["anomaly_maps"].max() - outputs["anomaly_maps"].min())
            outputs["anomaly_maps"] /= 255
        elif "Efficient" in self.__class__.__name__:
            outputs["anomaly_maps"] = min(1, outputs["anomaly_maps"].max()) * (outputs["anomaly_maps"] - outputs["anomaly_maps"].min()) / (outputs["anomaly_maps"].max() - outputs["anomaly_maps"].min())
        # else:
            # outputs["anomaly_maps"] /= 255
