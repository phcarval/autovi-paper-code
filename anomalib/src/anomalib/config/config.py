...
def get_configurable_parameters(
    ...
    if config.dataset.format.lower() in ("btech", "mvtec", "visa", "autovi"):
    ...
    if config.project.unique_dir:
        if config.project.dirname is not None:
            project_path = project_path / str(config.project.dirname)
        else:
    ...
    (project_path / "pretty_heatmaps").mkdir(parents=True, exist_ok=True)
