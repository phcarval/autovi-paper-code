...
class ImageVisualizerCallback(BaseVisualizerCallback):
    ...
    def on_test_batch_end(
        ...
        res = list(zip(paths, scores)).sort(key=lambda tup: tup[1])
        print(res)"""
        ...
        for i, image in enumerate(self.visualizer.visualize_batch(outputs)):
            ...
            if self.save_images:
                ...
                if type(image) is tuple:
                    self.visualizer.save(Path(self.image_save_path / "../pretty_heatmaps/").resolve() / filename.parent.name / filename.name, image[1])
                    image = image[0]
                self.visualizer.save(file_path, image)
            if self.log_images:
