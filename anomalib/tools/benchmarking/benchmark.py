...
def get_single_model_metrics(model_config: DictConfig | ListConfig, openvino_metrics: bool = False) -> dict:
    ...
    with TemporaryDirectory(dir='./.tmp') as project_path:
...
def sweep(
    ...
    if run_config.model_name in ["patchcore", "cflow"]:
        ...
        if model_config.model.input_size != (224, 224):
            model_config.model.input_size = (224, 244)  # go to next run
...
if __name__ == "__main__":
    ...
    run_configs = list(get_run_config(_sweep_config.grid_search))
    runs_folder = datetime.strftime(datetime.now(), "%Y_%m_%d-%H_%M_%S")
    ...
    compute_on_gpu(
                    ...
                    1,
                    _sweep_config.seed,
                    _sweep_config.writer,
                    runs_folder,
                    _sweep_config.compute_openvino)
